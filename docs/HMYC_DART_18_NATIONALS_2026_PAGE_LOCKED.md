...
| Score box size | Edit-mode R text (R1…R4 and every later R) = **computed `th.race-col` size** (R1 header label, live table **9px**). Explicit px on the input — iOS ignores `inherit` and blows up to 16px. Height stays **1em**. **Not** 16px / 22px circles. |
| Edit field value | Full stored cell: **`40 DNS`**, not bare `40`. Box **min-width = closed “00 DNS” text** so the whole code shows; when the race closes it is the same standard score text. |
