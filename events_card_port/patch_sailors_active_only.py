#!/usr/bin/env python3
"""Patch live api.py: /api/search?active=1 + /sailors page copy (active sailors search only)."""
...
if "active_on = int(active or 0) == 1" not in t:
    ...
    if anchor not in t:
        raise SystemExit("where_clause anchor not found")
    t = t.replace(anchor, active_exists, 1)
