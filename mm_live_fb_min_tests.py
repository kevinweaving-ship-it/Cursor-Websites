...
class LiptonMmCardUnitTest(unittest.TestCase):
    def test_helper_is_hard_scoped(self):
        ...
        self.assertIn("distM(rows[i].pos, mark) < 250", overlay)
        self.assertIn("racePlace <= 6", overlay)
        self.assertIn("[10, 12, 14]", overlay)
        self.assertIn("'Pin'", overlay)
        self.assertIn("(w - padX * 2) / spanAlong", overlay)
        self.assertIn("OVERALL", overlay)
        self.assertIn("RACE", overlay)
