...
(function () {
  ...
  function drawSpark(data) {
    ...
    for (; hourMs < lastMs; hourMs += HOUR_MS) {
      ...
      svg += '<text x="' + (hx + 1.2).toFixed(1) + '" y="8" fill="#334155" font-size="7" font-weight="700" font-family="Arial,Helvetica,sans-serif">' + fmtHm(hourMs) + "</text>";
    }
    var highD = "";
    var avgD = "";
    var hiCount = 0;
    ...
    html += '<div class="wx-spark-x"><span>' + (hours > 1.05 ? "older · swipe" : "1h") + "</span><span>now</span></div>";
    return html + "</div>";
  }
