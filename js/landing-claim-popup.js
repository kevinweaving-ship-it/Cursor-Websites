...
(function () {
  ...
  function injectCss() {
    ...
    s.textContent = [
      ...
      "#ssa-claim-modal.is-open{top:calc(var(--sa-header-h,80px) + env(safe-area-inset-top,0px))!important;right:0!important;bottom:0!important;left:0!important;flex-direction:column!important;align-items:center!important;justify-content:flex-start!important;}",
      ...
      "#ssa-claim-modal .ssa-claim-modal-panel{margin:auto!important;}",
      "@media screen and (orientation:portrait) and (max-width:767px){",
      "#ssa-claim-modal.is-open{padding-top:16px!important;padding-bottom:calc(20px + env(safe-area-inset-bottom,0px))!important;}",
      "#ssa-claim-modal .ssa-claim-modal-panel{margin:auto!important;}",
      "}",
      "body.ssa-std-claim-open .ssa-dev1-inject .ssa-popup-claim-slot,",
      "body.ssa-std-claim-open .ssa-dev1-inject .sa-looked-cta,",
      "body.ssa-std-claim-open .ssa-dev1-inject .sa-claim-banner,",
      "body.ssa-std-claim-open .ssa-dev1-inject .sa-looked-claim{",
      "visibility:hidden!important;opacity:0!important;pointer-events:none!important;z-index:0!important;",
      "}",
      "#popupOverlay.ssa-std-claim-popup{z-index:2147483646!important;}",
      "#loginRequiredSailorPreview{margin:0 0 1rem 0;}",
      "#loginRequiredSailorPreview .sa-looked-cta{display:flex;flex-direction:column;width:100%;max-width:100%;margin:0;padding:0;box-sizing:border-box;border:1px solid #c4a26f;border-radius:12px;overflow:hidden;background:#fefaf5;}",
      "#loginRequiredSailorPreview .sa-looked-claim{display:flex;flex-direction:column;width:100%;margin:0;padding:0;border:0;background:transparent;cursor:default;font:inherit;}",
      "#loginRequiredSailorPreview .sa-looked-claim-top{display:flex;flex-direction:column;align-items:center;gap:4px;padding:12px 14px 11px;text-align:center;}",
      "#loginRequiredSailorPreview .sa-looked-claim-ask{font-family:system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;font-weight:800;font-size:15px;line-height:1.15;color:#0a2351;}",
      "#loginRequiredSailorPreview .sa-looked-claim-sub{font-family:system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;font-weight:500;font-size:12px;line-height:1.2;color:#0a2351;}",
      "#loginRequiredSailorPreview .sa-looked-claim-bar{display:flex;align-items:center;justify-content:center;gap:8px;padding:11px 40px 11px 14px;background:#0a2351;position:relative;}",
      "#loginRequiredSailorPreview .sa-looked-claim-txt{font-family:system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;font-weight:800;font-size:12px;line-height:1;color:#fff;text-transform:uppercase;letter-spacing:.04em;}",
      "#loginRequiredSailorPreview .sa-looked-claim-go{position:absolute;right:10px;top:50%;transform:translateY(-50%);width:22px;height:22px;border-radius:999px;background:#c4a26f;color:#0a2351;display:inline-flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;}",
  ...
  function isPublicSailorProfilePage() {
    try {
      return /^\/sailor\/[^/]+/.test(String(location.pathname || ""));
    } catch (e) {
      return false;
    }
  }

  function openStdClaimForSlot(slot, info) {
    var live = sailorInfo(slot, info);
    try { window.__ssaClaimSailor = { sid: live.sid || "", name: live.name || "" }; } catch (_) {}
    window.__ssaClaimSlot = slot || null;
    closeModal();
    killLegacyOverlay();
  ...
  window.openLandingClaimPopup = function (info) {
    ...
    if (slot) openStdClaimForSlot(slot, info);
  };
  window.closeLandingClaimPopup = closeModal;
  window.openStdClaimForSlot = openStdClaimForSlot;
