...
- **EZVIZ Bing gold** — Bing Carport: MPEG dump + 640p @ 12fps. Stanford Front Garage: RTP HEVC decrypt + same 640p out (not Bing MPEG). Never 4K. If Stanford is gray/green, P is still 4/6 — see **`docs/EZVIZ_4K_TOO_HIGH_P.md`**. **`.cursor/rules/ezviz-bing-gold.mdc`**, **`docs/EZVIZ_BING_GOLD.md`**. Do not edit `carport_test` unless the user names Bing.
