...
- **URL protection / Lipton-dev isolation (HARD):** Public `/sailor/`, `/club/`, `/class/`, `/regatta/` (non-dev), `index.html`, `blank.html`, and `api.py` must not be overwritten by Lipton or tracking-dev work. Dev deploys use **allowlist only**: `bash sailingsa/deploy/deploy-tracking-dev2-live.sh` — never full frontend zip. See **`docs/URL_PROTECTION.md`** and **`.cursor/rules/url-protection-and-dev-isolate.mdc`**. Good restore reference: live `/root/backup_20260828_081731.tar.gz` (28 Aug).
