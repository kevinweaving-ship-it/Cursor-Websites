...
Lastmod (regattas): honest public change date — as_at_time (if not future),
else created_at, else past start/end. Never 2000-01-01 for a real public event.
Never lastmod=now just because the sitemap rebuilt.
Upcoming /regatta/{regatta_id} pages are included before results exist
(same URL as Live). Do not recreate sitemap-priority.xml.
"""
...
_PRIVATE_REGATTA_ID_RE = re.compile(
    r"(^live-|-dev2\b|tracking-dev|^test-|/admin)",
    re.I,
)
_PRIVATE_DUP_STATUS = frozenset(
    {"superseded", "deleted", "private", "admin", "test", "dev"}
)
...
def _is_public_regatta_id(regatta_id: str) -> bool:
    """Permanent public /regatta/{regatta_id} slug. Exclude admin/test/dev/private."""
    rid = (regatta_id or "").strip()
    if not rid or "/" in rid or " " in rid:
        return False
    if _PRIVATE_REGATTA_ID_RE.search(rid):
        return False
    return True


def _honest_regatta_lastmod(
    *,
    as_at_time: Any,
    created_at: Any,
    start_date: Any,
    ...
    """Truthful lastmod for a public event page. Priority matches the Upcoming→Live workflow.

    Source order (first usable, not-future date wins):
      1. as_at_time — real results/public snapshot
      2. created_at — Upcoming Event / regatta row pre-populated
      3. end_date / start_date — historical fallback only
    updated_at is not used: it does not move when results/as_at change.
    Never returns a future date. Never returns 2000-01-01 when a real date exists.
    """
...
def _fetch_regattas(cur, today: str) -> list[tuple[str, str]]:
    """Public /regatta/{regatta_id} pages, including Upcoming with zero results.

    Same permanent URL as Live. Does not wait for results rows. Does not invent IDs.
    """
    cur.execute(
        """
        SELECT
    ...
    for r in cur.fetchall() or []:
        ...
        if not _is_public_regatta_id(regatta_id):
            continue
        dup = (str(r.get("duplicate_status") or "")).strip().lower()
        if dup in _PRIVATE_DUP_STATUS:
            continue
        lastmod, _src = _honest_regatta_lastmod(
            as_at_time=r.get("as_at_time"),
