/**
 * Club-page venue weather + camera.
 * Live feeds come from js/live-cam-sources.js (how / where / why).
 * HYC uses source hyc-club as a permanent club-page card, not the MM reel.
 * The same source id can be an MM event-card video via SSALiveCam.mmVideo.
 * Super Admin show/hide decides if the public sees a live pass-through.
 * HMYC is a JPEG card only (hmyccam1.nwsza.net/latest.jpg). Never the ZVYC reel.
 */
(function () {
  'use strict';
  ...
  function injectCss() {
    ...
    s.textContent =
      ...
      '.club-page .club-live-media .club-hmyc-cam,' +
      '.club-page .club-live-media .club-live-cam{order:1!important;margin-top:10px;width:100%;}' +
      ...
      '.club-live-media.club-live-media--cam-off:not(.club-live-media--sa) .mm-lipton-reels,' +
      ...
      '.club-hmyc-cam .cam-shot{position:relative;aspect-ratio:16/9;width:100%;background:#000;overflow:hidden}' +
      '.club-hmyc-cam .cam-shot img{display:block;width:100%;height:118%;margin-top:-10%;object-fit:cover;object-position:center bottom;background:#000}' +
      '.club-hmyc-cam[hidden]{display:none!important}' +
      '/* HMYC_CAM_VALID_HIDE_v1 */';
  ...
  function liveCamSource(club) {
    var id = (club && club.liveCam) || '';
    if (window.SSALiveCam) {
      if (id && window.SSALiveCam.get(id)) return window.SSALiveCam.get(id);
      if (club && club.code) return window.SSALiveCam.forClub(String(club.code).toLowerCase());
    }
    return null;
  }
  ...
  function livePassVideo(club) {
    ...
    var src = liveCamSource(club);
    if (src && window.SSALiveCam && typeof window.SSALiveCam.mmVideo === 'function') {
      var video = window.SSALiveCam.mmVideo(src.id);
    ...
    return [
      {
        ...
        kind: 'webcam',
        live_pass: true,
        live_cam: club.liveCam || HYC_LIVE_CAM,
  ...
  function camEl() {
    return (
      document.getElementById(HYC_CAM_ID) ||
      document.getElementById(HMYC_CAM_ID) ||
      document.getElementById(MM_ID)
    );
  }

  function makeHmycCam(club) {
    var el = document.getElementById(HMYC_CAM_ID);
    if (!el) {
      el = document.createElement('section');
      el.id = HMYC_CAM_ID;
    }
    el.className = 'card club-live-cam club-hmyc-cam';
    el.setAttribute('aria-label', (club && club.camLabel) || 'HMYC club cam');
    el.setAttribute('data-mm-cam-status', (club && club.camStatusApi) || '/api/club-cam/hmyc');
    el.setAttribute('hidden', '');
    if (!el.querySelector('img')) {
      el.innerHTML = '<div class="cam-shot"><img alt="HMYC club cam" width="1600" height="900" decoding="async"></div>';
    }
    return el;
  }

  function setHmycCamVisible(ok) {
    var el = document.getElementById(HMYC_CAM_ID);
    if (!el) return;
  ...
  function paintHmycStill() {
    var img = document.querySelector('#' + HMYC_CAM_ID + ' img');
    if (!img) return;
    img.onload = function () {
      setHmycCamVisible(img.naturalWidth >= 32 && img.naturalHeight >= 16);
    };
    img.onerror = function () {
      setHmycCamVisible(false);
    };
    img.src = AGRO_CAM + '?t=' + Date.now();
  }

  function checkHmycStill() {
    return fetch('/api/club-cam/hmyc?_=' + Date.now(), {
      cache: 'no-store',
      ...
      .then(function (data) {
        if (data && data.valid === true) paintHmycStill();
        else setHmycCamVisible(false);
      })
      .catch(function () {
        setHmycCamVisible(false);
      });
  ...
  function startHmycStill() {
    setHmycCamVisible(false);
    checkHmycStill();
    if (window.__SSA_HMYC_STILL_TIMER__) return;
    window.__SSA_HMYC_STILL_TIMER__ = window.setInterval(function () {
      if (!document.hidden) checkHmycStill();
    }, (cfg() && cfg().stillIntervalMs) || 60000);
  }

  function makeHycCam(club) {
    var src = liveCamSource(club);
    var camId = (src && src.clubCardId) || HYC_CAM_ID;
    var streamName = (src && src.src) || 'hyc';
    var liveId = (src && src.id) || club.liveCam || HYC_LIVE_CAM;
    var el = document.getElementById(camId) || document.getElementById(HYC_CAM_ID);
    if (!el) {
      el = document.createElement('section');
      el.id = camId;
      el.className = 'card club-live-cam club-hyc-cam';
    }
    el.id = camId;
    el.classList.add('card', 'club-live-cam', 'club-hyc-cam');
  ...
  function fillHost(host, club) {
    ...
    if (club.livePass) host.appendChild(makeHycCam(club));
    else if (club.still) host.appendChild(makeHmycCam(club));
    else host.appendChild(makeMmCam(club));
    host.setAttribute('data-club-live-ready', '1');
    mountSaCamToggle(host, club);
    if (club.livePass) startHycGo2rtc();
    if (club.still) startHmycStill();
    return orderCards(host);
  }
  ...
  function applyCamPublic(host, data, sa) {
    ...
    if (cam) {
      if (!visible && !can) {
        ...
        cam.querySelectorAll('video-stream').forEach(function (el) {
          el.remove();
        });
        var cell = cam.querySelector('[data-cam]');
        if (cell) cell.removeAttribute('data-hyc-loading');
      } else {
        cam.removeAttribute('hidden');
        if (document.getElementById(HYC_CAM_ID)) startHycGo2rtc();
      }
    }
  ...
  function fetchCamStatus(club) {
    var src = liveCamSource(club);
    var api = (src && src.statusApi) || (club && club.camStatusApi) || '/api/club-cam/hyc';
    return fetch(api + (api.indexOf('?') >= 0 ? '&' : '?') + '_=' + Date.now(), {
      credentials: 'include',
      cache: 'no-store',
    }).then(function (r) {
      return r && r.ok ? r.json() : null;
  ...
  function mountSaCamToggle(host, club) {
    ...
    btn.addEventListener('click', function () {
      ...
      var src = liveCamSource(club);
      var patch = (src && src.saPatch) || '/api/super-admin/club-cam/hyc';
      btn.disabled = true;
      fetch(patch, {
        method: 'PATCH',
        credentials: 'include',
  ...
  function boot() {
    ...
    if (!club) return;
    injectCss();
    if (!club.livePass && !club.still) ensureCssLink();
