...
def paginate_fleet_htmls(fleets: list[dict], orient: str) -> list[dict]:
    ...
    Estimates are slightly conservative so Chromium never has to split a
    fleet header from its results table.
    """
    page_mm = 188 if orient == "landscape" else 248
    ...
    for i, fleet in enumerate(fleets):
        ...
        rm = (5.4 if orient != "landscape" else row_mm) if compact else row_mm
        hdr = 24.0 if compact else 16.0
        h = hdr + n_rows * rm
        if h > page_mm:
...
def write_event_pdfs(
    *,
    ...
    try:
        ...
        cc_portrait = False
    orient = "portrait" if cc_portrait else orientation_from_fleet_htmls(htmls)
    ...
    with _lock_for(slug):
        ...
        for fleet in fleets:
            ...
            if not child_html_src or not (cslug or pdf_slug):
                continue
            child_orient = "portrait" if cc_portrait else orientation_from_fleet_htmls([child_html_src])
...
if __name__ == "__main__":
    ...
    assert "ssa-print-new-page" not in (b["html"] or ""), "420 + Extra must share page 1"
