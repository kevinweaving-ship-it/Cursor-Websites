...
(function () {
    ...
    thead.querySelectorAll("th.race-col").forEach(function (th) {
      ...
      th.addEventListener("click", function (ev) {
        ...
        if (wasClosed) {
          ...
          syncClosedColumn(table, key);
        }
      });
    });
  }

  function raceHeaderFont(table) {
    var th =
      (table && table.querySelector("thead th.race-col")) ||
      document.querySelector("th.race-col");
    if (!th) return { size: "9px", family: "inherit", weight: "inherit", line: "1.2" };
    var cs = window.getComputedStyle(th);
    return {
      size: cs.fontSize || "9px",
      family: cs.fontFamily || "inherit",
      weight: cs.fontWeight || "inherit",
      line: cs.lineHeight || "1.2",
    };
  }

  function paintScoreInputFont(root) {
    var table = root && root.querySelector ? root : null;
    if (root && root.closest) table = root.closest("table") || table;
    if (!table || !table.querySelector) {
      table = document.querySelector("table.fleet-results-table");
    }
    if (!table) return;
    ...
    table.querySelectorAll(".club-score-input, .wc-result-field-input").forEach(function (inp) {
      ...
      inp.style.setProperty("height", f.size, "important");
  ...
  function scorePaint(raw) {
    ...
    if (!src) {
      ...
      classes.push("code");
      if (discarded) classes.push("disc");
  ...
  function syncRaceHide(td, raw) {
    ...
    if (!hide) {
      ...
      hide.className = "wc-sa-edit-hide";
      td.insertBefore(hide, td.firstChild);
    }
    hide.innerHTML = painted.html;
    td.setAttribute("data-live-cell", painted.cell);
  }
  ...
  function paintRaceCell(td, raw) {
    ...
    var painted = scorePaint(cell);
    td.classList.remove("disc", "code", "score-counts");
    td.classList.add("race-col");
    painted.classes.forEach(function (c) {
      td.classList.add(c);
    });
    td.innerHTML = painted.html;
    td.setAttribute("data-live-cell", painted.cell);
  }
  ...
  function wireCell(td, resultId) {
    ...
    syncRaceHide(td, current);
    Array.prototype.slice.call(td.childNodes).forEach(function (node) {
      if (node.nodeType === 1 && node.classList && node.classList.contains("wc-sa-edit-hide")) return;
      if (node.parentNode) node.parentNode.removeChild(node);
    });
  ...
  function sailedCountFromTable(table) {
    var sec = table && table.closest ? table.closest(".fleet-section") : null;
    var n = parseInt(sec && sec.getAttribute("data-races-sailed"), 10) || 0;
    var line = sec && sec.querySelector(".sailed-line");
    var m = line && String(line.textContent || "").match(/Sailed:\s*(\d+)/i);
    if (m) n = Math.max(n, parseInt(m[1], 10) || 0);
    return Math.max(n, fleetRaceCount(table), 1);
  }

  function ensureOpenRaces(table) {
    var n = sailedCountFromTable(table);
    var i;
    for (i = 1; i <= n; i++) ensureRace(table, i);
  }
