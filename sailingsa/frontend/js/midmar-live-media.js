...
(function () {
  ...
  var CLUB_PROFILES = {
    HMYC: { weatherClub: "HMYC", weatherStation: "agromet-midmar" },
  };
  var SEED_MEDIA_SLUGS = { "2026-09-19-hmyc-midmar-cup": true };
  ...
  function dateFirstClub(rid) {
    var m = String(rid || "").match(/^(\d{4}-\d{2}-\d{2})-([a-z0-9]+)/);
    return m ? m[2].toUpperCase() : "";
  }

  function fleetBoatClass() {
    var a = document.querySelector('.class-header a[href*="/class/"]');
    var href = a && a.getAttribute("href");
    var m = href && href.match(/\/class\/([^/?#]+)/);
    if (m) {
      return m[1]
        .split("-")
        .map(function (w) {
    ...
    var t = document.querySelector(".fleet-title-public-nav, .fleet-title-row");
    return String((t && t.textContent) || "")
      .replace(/\s*Fleet\s*$/i, "")
      .trim();
  }
  ...
  function currentEvent() {
    ...
    var club =
      (page && (page.getAttribute("data-club-live-cards") || "").toUpperCase()) ||
      dateFirstClub(rid);
    if (!club && page && (page.getAttribute("data-hmyc-live") === "1" || page.getAttribute("data-midmar-cup") === "1")) {
      club = "HMYC";
    }
    if (!rid || !club || !CLUB_PROFILES[club]) return null;
    return {
      rid: rid,
      club: club,
      boatClass: fleetBoatClass(),
      emptyMedia: !SEED_MEDIA_SLUGS[rid],
    };
  }
  ...
  function injectCss() {
    ...
    s.textContent = [
      ...
      ".regatta-page > .midmar-live-media .mm-lipton-reels-brand{display:none!important;}",
      ...
      ".midmar-live-media .mm-lipton-reels-tile--reel:not(.mm-midmar-cup-thumb) .mm-lipton-reels-thumb{border-left:0!important;border-right:0!important;border-radius:0!important;background:#000;}",
  ...
  function makeMmCard() {
    ...
    if (ev.emptyMedia) {
      mm.setAttribute(
        "data-mm-initial",
        JSON.stringify({ enabled: true, feed_source: "midmar", videos: [] })
      );
      mm.innerHTML =
    ...
    if (save) {
      save.addEventListener("click", function () {
        ...
        posterFromFile(file).then(function (meta) {
          ...
          return fetch("/api/super-admin/regatta/" + encodeURIComponent(RID) + "/mm-clips", {
            method: "POST",
            body: fd,
            credentials: "include",
          }).then(function (r) {
  ...
  function boot() {
    ...
    loadScript("/js/midmar-leaderboard.js?v=clubcards1");
    loadScript("/js/midmar-media-rotate.js?v=clubcards1");
