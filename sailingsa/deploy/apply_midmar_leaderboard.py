...
def main() -> None:
    ...
    mm = MM.read_text()
    for old_ver in ("mmlb1", "mmlb2", "mmlb3", "mmlb4", "mmlb5"):
        needle = "midmar-leaderboard.js?v=" + old_ver
        if needle in mm:
            mm = mm.replace(needle, "midmar-leaderboard.js?v=mmlb6")
            MM.write_text(mm)
            print("MM_VER mmlb6")
            break
    mm = MM.read_text()
    ...
    bleed_fix = ".regatta-page > .midmar-live-media .ssa-regatta-slot-card{width:100%;max-width:100%;margin-left:0;margin-right:0;border:2px solid #001f3f;border-radius:8px;box-sizing:border-box;}"
    if bleed in mm:
        mm = mm.replace(bleed, bleed_fix)
        MM.write_text(mm)
        print("MM_WX_WIDTH_MATCH")
        mm = MM.read_text()
    if MARK in mm:
    ...
    api = API.read_text()
    api2 = api.replace("midmar-leaderboard.js?v=mmlb1", "midmar-leaderboard.js?v=mmlb6")
    api2 = api2.replace("midmar-leaderboard.js?v=mmlb2", "midmar-leaderboard.js?v=mmlb6")
    api2 = api2.replace("midmar-leaderboard.js?v=mmlb3", "midmar-leaderboard.js?v=mmlb6")
    api2 = api2.replace("midmar-leaderboard.js?v=mmlb4", "midmar-leaderboard.js?v=mmlb6")
    api2 = api2.replace("midmar-leaderboard.js?v=mmlb5", "midmar-leaderboard.js?v=mmlb6")
    ...
    def _pin(m: re.Match) -> str:
        ...
        if "midmar-leaderboard.js" in rest:
            return tag
        return tag + '<script src="/js/midmar-leaderboard.js?v=mmlb6" defer></script>'
