#!/usr/bin/env python3
"""Install Midmar Leader Board black rank dividers and cache-bust mmlb14."""
...
def main() -> None:
    src = SRC.read_text()
    if "border-bottom:1px solid #000" not in src or "midmar-leaderboard-css-v11" not in src:
        raise SystemExit("SRC_BAD")
    DEST.write_text(src)
    DEST.chmod(0o644)
    print("JS_OK", DEST.stat().st_size)

    olds = ("mmlb6", "mmlb7", "mmlb8", "mmlb9", "mmlb10", "mmlb11", "mmlb12", "mmlb13")
    mm = MM.read_text()
    mm2 = mm
    for old in olds:
