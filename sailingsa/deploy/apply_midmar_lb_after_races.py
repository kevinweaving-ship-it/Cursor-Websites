#!/usr/bin/env python3
"""Install Midmar Leader Board 'after N Races' title and cache-bust mmlb12."""
...
def main() -> None:
    src = SRC.read_text()
    if "Leader Board after " not in src or "function titleAfter" not in src:
        raise SystemExit("SRC_BAD")
    DEST.write_text(src)
    DEST.chmod(0o644)
    print("JS_OK", DEST.stat().st_size)

    olds = ("mmlb6", "mmlb7", "mmlb8", "mmlb9", "mmlb10", "mmlb11")
    mm = MM.read_text()
    mm2 = mm
    for old in olds:
