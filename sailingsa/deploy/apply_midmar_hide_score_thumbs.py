#!/usr/bin/env python3
"""Remove the two most recent Midmar results / Leader Board media thumbs."""
...
# Public order after pinned trophy: results sheet, then Leader Board strip.
REMOVE_IDS = {"mmclip-8a93a16fab9d", "mmclip-f3145e8c0f55"}
...
def main() -> None:
    ...
    bak = path.with_name(f"clips.json.bak.rmscore2.{ts}")
    ...
    for v in videos:
        ...
        keep.append(v)
    if len(removed) != 2:
        raise SystemExit("REFUSE expected 2 score thumbs, got " + str(len(removed)))
