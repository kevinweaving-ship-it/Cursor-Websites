#!/usr/bin/env python3
"""Install Midmar Leader Board dark-blue underlines and cache-bust mmlb11."""
...
def main() -> None:
    src = SRC.read_text()
    if "color:#0000ee" not in src or "class-link-valid" not in src or "midmar-leaderboard-css-v10" not in src:
        raise SystemExit("SRC_BAD")
    DEST.write_text(src)
    DEST.chmod(0o644)
    print("JS_OK", DEST.stat().st_size)

    olds = ("mmlb6", "mmlb7", "mmlb8", "mmlb9", "mmlb10")
    mm = MM.read_text()
    mm2 = mm
    for old in olds:
