...
# Known SAS list typos (source_event_id → ISO end_date). Prefer these over scraped end year.
SAS_END_DATE_OVERRIDES = {
    "361162": "2026-05-31",  # DSO Appointments: SAS shows 2029; real window is May 2026
}


def sanitize_event_end_date(start_iso: str | None, end_iso: str | None, source_event_id: str | None = None) -> str | None:
    """Fix obvious end-date nonsense: known overrides + same-month year typos (e.g. May 2026–May 2029)."""
    if source_event_id and source_event_id in SAS_END_DATE_OVERRIDES:
        return SAS_END_DATE_OVERRIDES[source_event_id]
    if not start_iso or not end_iso:
        return end_iso
    try:
        sy, sm, sd = (int(x) for x in start_iso.split("-")[:3])
        ey, em, ed = (int(x) for x in end_iso.split("-")[:3])
    except (TypeError, ValueError):
        return end_iso
    if ey <= sy:
        return end_iso
    # Same calendar month, end day a short span after start day → SAS year typo (keep start year).
    if em == sm and 0 < (ed - sd) <= 14:
        return f"{sy:04d}-{em:02d}-{ed:02d}"
    return end_iso


def parse_start_end(text_block: str) -> tuple[str | None, str | None, str | None, str | None]:
    """Return (start_date, end_date, start_time, end_time) from DATE_RE matches.

    Only the first date and an optional end date on the *same* range line are used.
    Matches from the next event card (farther down the HTML block) are ignored —
    that bug made VULCAN inherit DF95's Fri 18 Sep end date.
    """
    ...
    first = matches[0]
    start_date = to_iso(first)
    start_time = to_time(first)
    # Same-line end: second full date must sit close after the first (e.g. "09:00 - Thu 31 May 2029 09:00")
    MAX_SAME_LINE_GAP = 64
    if len(matches) > 1 and (matches[1].start() - first.end()) <= MAX_SAME_LINE_GAP:
        end_date = to_iso(matches[1])
        end_time = to_time(matches[1])
    else:
        # e.g. "Sat 12 Sep 2026 09:00 - 09:00" → same-day
        end_date = start_date
        end_time = start_time
        trail = text_block[first.end(): first.end() + 20]
        tm = re.match(r"\s*[-–—]\s*(\d{2}:\d{2})\b", trail)
        if tm:
            end_time = tm.group(1)
...
def parse_detail_html(html: str, base_url: str) -> dict:
    ...
    if date_matches:
        ...
        if len(date_matches) > 1:
            ...
            trail = html_clean[date_matches[0].end(): date_matches[0].end() + 20]
            tm = re.match(r"\s*[-–—]\s*(\d{2}:\d{2})\b", trail)
            if tm:
                t = tm.group(1)
                out["end_time"] = t if t.count(":") >= 2 else t + ":00"
        out["end_date"] = sanitize_event_end_date(out.get("start_date"), out.get("end_date"), None) or out.get("end_date") or ""
    else:
        ...
        if range_m:
            ...
            out["end_date"] = sanitize_event_end_date(out["start_date"], out["end_date"], None) or out["end_date"]
...
def parse_list_html(html: str, is_past: bool) -> list[dict]:
    ...
    for m in EVENT_LINK_RE.finditer(html):
        ...
        if "/events/list" in url and not re.search(r"/events/\d", url):
            continue
        ...
        eid_for_fix = sas_id or ext_id
        end_iso = sanitize_event_end_date(start_iso, end_iso, eid_for_fix)
    ...
    for m in EVENT_LINK_REL_RE.finditer(html):
        ...
        block_text = extract_text_only(block)
        start_iso, end_iso, start_t, end_t = parse_start_end(block)
        end_iso = sanitize_event_end_date(start_iso, end_iso, eid)
        venue_line = ""
        category_line = ""
        for cat in LIST_CATEGORY_STRINGS:
            if cat.lower() in block_text.lower():
